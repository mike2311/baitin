  ...
  imports: [
    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: (configService: ConfigService) => {
        ...
        const port = parseInt(configService.get<string>('DATABASE_PORT') || process.env.DATABASE_PORT || '5432', 10)
        const username = configService.get<string>('DATABASE_USER') || process.env.DATABASE_USER || 'baitin_dev'
        const password = configService.get<string>('DATABASE_PASSWORD') || process.env.DATABASE_PASSWORD || 'baitin_dev_password'
        const database = configService.get<string>('DATABASE_NAME') || process.env.DATABASE_NAME || 'baitin_poc_dev'
        ...
        password,
        database,
        ssl: host.includes('supabase.co') ? { rejectUnauthorized: false } : false,
        entities: [
          User,
          Item,
          Customer,

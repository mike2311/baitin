...
w_inv_no = "HI/0170/99"
select minvhd.inv_no, date, cust_no, cust_name, addr_1,addr_2,;
	addr_3, addr_4, minvhd.conf_no, ship, loading, dest, po_no,;
		cover,alltrim(mline(minvdt.desp_memo,1,1)) as "desp_memo", ctn, qctn, qty, net, wt, cube, dim;
		 from minvhd inner join minvdt on minvhd.inv_no = minvdt.inv_no;
		 where minvhd.inv_no = w_inv_no into cursor minvhd_cursor
return		
	...
	join_cursor.addr_2,join_cursor.addr_3,join_cursor.addr_4, conf_no, ship, loading, dest, po_no,;
		cover, desp_memo, ctn, qctn, qty, iif(nvl(mcustom.wt_unit,0)=2, net/2.2, net) as "net", ;
			...
			from join_cursor left outer join mcustom on join_cursor.cust_no;
				= mcustom.cust_no into table temp_packlist_table2
	
select wpacklist
append from temp_packlist_table2

select temp_packlist_table2
use 
delete file temp_packlist_table2.*

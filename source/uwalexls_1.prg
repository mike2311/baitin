...
* locate_header_record
   do while at("QUOTE", eole.cells(w_row,1) = 0  or w_row < 10)
        w_row=w_row+1
   enddo 
   if w_row < 10
      w_header_row=w_row
   else
      @12,5 say "Invalid xls file format. Update Program stopped"
      return
   endif      
xxx
...
Procedure update_moe
      ...
      select mqtybrk
      locate for alltrim(mqtybrk.oe_no)== alltrim(w_oe_no) 
      if !eof()
         select mqtybrk
...
*         replace moe.rp_1      with numval(&n_retail)
           ...
           pack_1= "  "+iif(w_pack_pc_1=0,"   ",alltrim(str(w_pack_pc_1))+" ")+w_pack_desp_1
...
Procedure Set_vendor_maker
				...
				if !empty(&n_maker)
				   replace moe.maker with &n_maker
				else

...
select a.*, nvl(b.port_name,space(50)) as port_name, nvl(b.port_grp,"") as port_grp from vTmpQtyBrk a left outer join mshipmark b on a.cust_no == b.cust_no and a.port == b.port into cursor vTmpQtyBrk
select a.*, nvl(b.row_ref," ") as row_ref, nvl(b.port,space(20)) as port from vTmpMoe a left outer join vTmpQtyBrk b on a.oe_no == b.oe_no and a.item_no == b.item_no into cursor vTmpMoe group by a.oe_no, row_ref, a.vendor_no, a.item_no order by a.oe_no, row_ref desc, a.vendor_no, a.item_no
select wpobrkgrid_1
...
strOeRow = ""
do while !eof()
        select * from vTmpMoe into cursor coe where alltrim(oe_no)==alltrim(wpobrkgrid_1.oe_no) order by row_ref desc, vendor_no, item_no 
...
Procedure Set_qtybrk
               Select * from vTmpQtyBrk into cursor vTmpQtyBrk_1
